...
import type { SmallFamilyFormData, FamilyMember, Leader } from '../../types/smallFamilies';
import './CreateSmallFamilyModal.css';

interface CreateSmallFamilyModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

const defaultFormData = (): SmallFamilyFormData => ({
  name: '',
  responsible_id: null,
  cep: '',
  street: '',
...
export default function CreateSmallFamilyModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateSmallFamilyModalProps) {
  const [formData, setFormData] = useState<SmallFamilyFormData>(defaultFormData());
  ...
  const loadLeaders = async () => {
    ...
    try {
      const response = await api.get('/small-families');
      setLeaders(response.data.families || []);
    } catch (error) {
  ...
  const handleChange = (field: keyof SmallFamilyFormData, value: any) => {
  ...
  const validateForm = (): boolean => {
    if (!formData.name.trim()) {
      showWarning('Informe o nome da Pequena Família');
      return false;
    }
    if (!formData.responsible_id) {
      showWarning('Selecione o responsável pela Pequena Família');
      return false;
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post('/small-families/create', formData);
      showSuccess('Pequena Família criada com sucesso!');
      onSuccess();
      onClose();
    } catch (error: any) {
      console.error('Erro ao criar Pequena Família:', error);
      showError(error.response?.data?.error || 'Erro ao criar Pequena Família');
    } finally {
      setLoading(false);
    }
  };
  ...
  const footer = (
    <>
      ...
      <button type="submit" form="create-family-form" className="btn btn-primary btn-md" disabled={loading}>
        {loading ? 'Salvando...' : 'Criar Pequena Família'}
      </button>
    </>
  );
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Criar Pequena Família"
      subtitle="Cadastro de nova Pequena Família"
      footer={footer}
      maxWidth="900px"
    >
      <form id="create-family-form" className="create-family-form" onSubmit={handleSubmit}>
        ...
        <div className="form-section">
          ...
          <div className="form-row">
            <div className="form-group">
              <label htmlFor="name">Nome da Pequena Família *</label>
              <input
                id="name"
                type="text"
                ...
                placeholder="Ex: Pequena Família do Bairro Centro"
                required
              />
            </div>
          </div>
          ...
          <div className="form-row">
            <div className="form-group">
              <label htmlFor="responsible">Responsável pela Pequena Família *</label>
              <select
                id="responsible"
                className="form-input"
